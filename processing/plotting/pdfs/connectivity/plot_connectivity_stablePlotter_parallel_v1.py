...
for ii in pdfs_to_plot_indices:
    ...
    #print(f"{np.shape(connectivity_pdf_list[ii])}")
    #print(f"{np.shape(release_counts_per_cell_list[ii])}")
    #print(f"{np.shape(release_counts_per_cell_list[ii].T)}")
    #print(f"{np.shape(release_counts_per_cell_list[ii][:, np.newaxis])}")
    #print(f"{np.shape(release_counts_per_cell_list[ii].T[:, np.newaxis])}")
    
    # recall paul's lesson; np.newaxis really means None, and this is about the pointwise expansion business... very convenient
    #connectivity_pdf_list[ii] = connectivity_pdf_list[ii] / release_counts_per_cell_list[ii][:, np.newaxis]
    connectivity_pdf_list[ii] = connectivity_pdf_list[ii] / release_counts_per_cell_list[ii][0][:, np.newaxis] # Now need to access element 0..... 
    
    print(f"{np.shape(connectivity_pdf_list[ii])}")
    
    if np.amax(connectivity_pdf_list[ii]) > pdf_max_val:
        pdf_max_val = np.amax(connectivity_pdf_list[ii])
    if np.amin(np.ma.masked_invalid(connectivity_pdf_list[ii])) < pdf_min_val:
        pdf_min_val = np.amin(np.ma.masked_invalid(connectivity_pdf_list[ii]))
