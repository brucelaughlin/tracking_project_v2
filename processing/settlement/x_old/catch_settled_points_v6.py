    ...
    for tracking_output_file_pre in tracking_output_files:
        ...
        for time_dex in range(first_settle_dex,last_settle_dex):
            ...
            for polygon_dex in range(num_polygons):
                ...
                #stagnant_list_lon = np.zeros(num_particles, dtype=bool)
                #stagnant_list_lat = np.zeros(num_particles, dtype=bool)
               
               
               # for t_hist in range(0,time_dex):
               #     stagnant_list_lon += particles_lon_all[:,t_hist] == particles_lon_all[:,t_hist + 1]
               #     stagnant_list_lat += particles_lat_all[:,t_hist] == particles_lat_all[:,t_hist + 1]
                    #d4 = particles_lon_all[:,t_hist] == particles_lon_all[:,t_hist + 1]
              
               # offset_lon = particles_lon_all[:,1:] - particles_lon_all[:,0:-1]
               # offset_lat = particles_lat_all[:,1:] - particles_lat_all[:,0:-1]
                
                
                #offset_lon = np.subtract(particles_lon_all,particles_lon_all)
                #offset_lat = np.subtract(particles_lat_all,particles_lat_all)
                #offset_lon = particles_lon_all - particles_lon_all
                #offset_lat = particles_lat_all - particles_lat_all
                
                #offset_lon = (particles_lon_all[:,1:] - particles_lon_all[:,0:-1]).astype(bool)
                #offset_lat = (particles_lat_all[:,1:] - particles_lat_all[:,0:-1]).astype(bool)
...
#                    num_outofbounds += 1
    ...
    save_output_full_path = os.path.join(save_output_directory, save_output_file_name + "_pld_{}_{}_noLoops".format(pld_array[pld_dex,0],pld_array[pld_dex,1]))
