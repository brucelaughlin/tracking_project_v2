...
for pld_dex in range(len(pld_array)):
    ...
    for tracking_output_file_pre in tracking_output_files:
        ...
        O2_threshold_breach_array = np.zeros((len(O2_limit_list),np.shape(particles_O2_all)[0],np.shape(particles_O2_all)[1]))
        pH_threshold_breach_array = np.zeros((len(pH_limit_list),np.shape(particles_pH_all)[0],np.shape(particles_pH_all)[1]))
        ...
        for particle_num in range(num_particles):
            if drifting_actively_array[particle_num,0] != False: # Only process non-stationary settlers
                ...
                #for timestep in range(np.shape(drifting_actively_array)[1]):
        ...
        for particle_num in range(num_particles):
            ...
            if release_boxes[particle_num] == 0:
                ...
                for array_dex in array_dex_list:
                    ...
                    #print(int(settlement_boxes[particle_num])-1)
