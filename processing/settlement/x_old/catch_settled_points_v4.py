...
for pld_dex in range(len(pld_array)):
    ...
    for tracking_output_file_pre in tracking_output_files:
        ...
        good_indices = particles_lon_all != dummy_value # For time averages
...
#        for timestep in range(timesteps_full_run): # I had been using the wrong "number of timesteps", but my code still should have caught this...!?
       ...
       # pH_threshold_breach_array = np.ma.array(pH_threshold_breach_array, mask = np.isnan(pH_threshold_breach_array))
        ...
        particle_drift_times = np.sum(good_indices[:,0:timesteps_full_run], axis=1)
        ...
        #particle_arrays_O2[:,limit_dex] = np.sum(O2_threshold_breach_array[limit_dex,:,:]
        #particle_arrays_pH[:,limit_dex] += pH_threshold_breach_array[limit_dex,:,timestep]
        
        #for limit_dex in range(len(O2_limit_list)):
        #    particle_arrays_O2[:,limit_dex] += O2_threshold_breach_array[limit_dex,:,timestep]
        #for limit_dex in range(len(pH_limit_list)):
        #    particle_arrays_pH[:,limit_dex] += pH_threshold_breach_array[limit_dex,:,timestep]
