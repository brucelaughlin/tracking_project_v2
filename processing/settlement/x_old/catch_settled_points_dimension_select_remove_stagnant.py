    ...
    for tracking_output_file_pre in tracking_output_files:
        ...
        for time_dex in range(first_settle_dex,last_settle_dex):
            ...
            for polygon_dex in range(num_polygons):
                ...
                # Wait a minute - I believe that "stationary settlers" are in a different category than "stagnant drifters".  I am feeling as though the latter
                # Should simpy be discarded, `

                d4[d3] = False

                # Maybe need to switch to algorithm that does all removal once the drifting_actively_array has been calculated??

                if time_dimension == 0:
                    drifting_actively_array[:,d4] = False
                    #drifting_actively_array[:,d1*d2*d3*d4] = False
        ...
        # ALSO, this now stores False for all times after the first instance of False, which we'll need for exposure
        if time_dimension == 0:
            for particle_num in range(num_particles):
                if drifting_actively_array[0,particle_num] == False:
                    drifting_actively_array[:,particle_num] = False
                #else:  ## Silly - Patrick's data doesn't have exposure data, so we don't need this
                #    if np.shape(np.where(np.logical_not(drifting_actively_array[:,particle_num]))[0])[0] > 0:
                #        last_valid_drift_dex = np.where(np.logical_not(drifting_actively_array[:,particle_num]))[0][0]
                #        if last_valid_drift_dex < np.shape(drifting_actively_array)[0]
                #            drifting_actively_array[last_valid_drift_dex+1:,particle_num] = False
        else:
            for particle_num in range(num_particles):
                if drifting_actively_array[particle_num, 0] == False:
                    ...
                    if np.shape(np.where(np.logical_not(drifting_actively_array[particle_num,:]))[0])[0] > 0:
                        last_valid_drift_dex = np.where(np.logical_not(drifting_actively_array[particle_num,:]))[0][0]
                        if last_valid_drift_dex < np.shape(drifting_actively_array)[0]
                            drifting_actively_array[particle_num,last_valid_drift_dex+1:] = False
