# Chris wants me to exclude floats which never leave their seed polygon.  For now I can only think of doing this by first going over the entire file
# and storing any polygons that a float has entered, and then go through my previous algorithm, which addresses each pld individually.
# This seems redundant and twice as slow - would be best to do it all in one loop.  But, that takes more thinking
...
script_version = "singleFileParallel_plds_seasons_test3"
...
# For testing, I removed the "--" in these arguments; revert when finalized
...
pdrake_file_switch = 0   # This is a hard-code hack to turn off this switch.  Fix in production, but still might need for pDrake files
...
#non_stagnant_particle_boolean_vector = np.zeros(num_particles)==0
#non_stagnant_particle_boolean_vector = np.zeros(num_particles)==1

print(f"Start script:  num_stagnant: {np.sum(stagnant_particle_boolean_vector):>8}")
...
for polygon_dex in range(num_polygons):
    ...
    release_polygons[np.logical_and(particles_inside_flags,particle_safety_mask)] = polygon_dex + 1
...
# For testing, comment this out.  It was also there for testing, but now I am testing with only one seed location, so no need to report all other cells empty
#print("--------------------------------------------")
...
for time_dex in range(first_time_dex,pld_list_timesteps[-1][-1]):
    ...
    #if pld_dex != num_plds - 1:
    #    continue

    #if time_dex > 1:
    #    break

    #print(f'timestep {time_dex:>3}/{pld_list_timesteps[-1][-1] -1}')
    ...
    particle_safety_mask = np.ones(num_particles) == 1
    #particle_safety_mask = np.ones(num_particles, dtype=bool)
...
#    stationary_settler_array = np.ones((num_particles,num_polygons), dtype=bool) 
    ...
    current_timestep_polygon_occupation = np.empty(num_particles)
    current_timestep_polygon_occupation[:] = np.nan
...
#    print("timestep {}/{}".format(time_dex,last_settle_dex-1))
    ...
    for polygon_dex in range(num_polygons):
        ...
        if np.sum(particles_inside_flags) > 0:
            print(np.unique(points_lon_lat[:,0][particles_inside_flags].data))
            print(np.sum(particles_inside_flags))
            #if len(np.unique(points_lon_lat[:,0][particles_inside_flags]).data) > 1:
            #print(np.unique(points_lon_lat[:,0][particles_inside_flags]).data)

        #if polygon_dex == 206:
        #    break

        #stagnant_particle_boolean_vector = np.logical_and(particle_seed_polygon_vector == particles_inside_flags * (polygon_dex + 1), stagnant_particle_boolean_vector)
        ...
        #indices_to_evaluate_for_stagnation = particle_seed_polygon_vector == particles_inside_flags * (polygon_dex + 1)
    
        #if np.sum(indices_to_evaluate_for_stagnation) > 0:
        #    print(f"{np.sum(indices_to_evaluate_for_stagnation)}")


        #stagnant_particle_boolean_vector[indices_to_evaluate_for_stagnation] = np.logical_and(particle_seed_polygon_vector[indices_to_evaluate_for_stagnation]  == particles_inside_flags[indices_to_evaluate_for_stagnation] * (polygon_dex + 1), stagnant_particle_boolean_vector[indices_to_evaluate_for_stagnation])
        

        current_polygon_occupation_indices = np.logical_and(particles_inside_flags,particle_safety_mask)  
        #current_polygon_occupation_indices_noStagnation = np.logical_and(np.logical_and(particles_inside_flags, particle_safety_mask), np.logical_not(non_stagnant_particle_boolean_vector))  
        #current_polygon_occupation_indices_noStagnation = np.logical_and(np.logical_and(particles_inside_flags, particle_safety_mask), non_stagnant_particle_boolean_vector)  
        current_polygon_occupation_indices_noStagnation = np.logical_and(np.logical_and(particles_inside_flags, particle_safety_mask), np.logical_not(stagnant_particle_boolean_vector))  

        current_timestep_polygon_occupation[current_polygon_occupation_indices] = polygon_dex + 1

        if currently_within_pld_switch:
        
            settlement_polygons[current_polygon_occupation_indices,pld_dex] = polygon_dex + 1
            settlement_times[current_polygon_occupation_indices,pld_dex] = time_dex + 1

            settlement_polygons_noStagnation[current_polygon_occupation_indices_noStagnation,pld_dex] = polygon_dex + 1
            settlement_times_noStagnation[current_polygon_occupation_indices_noStagnation,pld_dex] = time_dex + 1
...
#        counter_array[:] += current_polygon_occupation_indices 
        #counter_array[:,file_number] += current_polygon_occupation_indices 

    

#        non_stagnant_particle_boolean_vector = np.logical_and(release_polygons == current_timestep_polygon_occupation, non_stagnant_particle_boolean_vector)
#        #non_stagnant_particle_boolean_vector = np.logical_and(np.logical_and(release_polygons, current_timestep_polygon_occupation), non_stagnant_particle_boolean_vector)

    #print(f"Number of stagnant particles: {np.sum(non_stagnant_particle_boolean_vector)}")

    print(f"timestep {time_dex:>3}/{pld_list_timesteps[-1][-1] -1}, num_stagnant: {np.sum(stagnant_particle_boolean_vector):>8}")
    #print(f"time_dex: {time_dex:>3}, num_stagnant: {np.sum(non_stagnant_particle_boolean_vector):>8}")
