...
#    print("timestep {}/{}".format(time_dex,last_settle_dex-1))
    ...
    for polygon_dex in range(num_polygons):
        ...
        #current_polygon_occupation_indices_noStagnation = np.logical_and(np.logical_and(particles_inside_flags, particle_safety_mask), stagnant_particle_boolean_vector)  
...
#        counter_array[:] += current_polygon_occupation_indices 
        ...
        stagnant_particle_boolean_vector = np.logical_and(release_polygons == current_timestep_polygon_occupation, stagnant_particle_boolean_vector)
        #stagnant_particle_boolean_vector = np.logical_and(np.logical_and(release_polygons, current_timestep_polygon_occupation), stagnant_particle_boolean_vector)
...
#save_output_full_path = os.path.join(save_output_directory, save_output_file_name + f"_pld_{pld_list[pld_dex,0]}_{pld_list[pld_dex,1]}_version_{script_version}_{polygon_string}")
