...
input_model_nc_file = "/home/blaughli/symbolic_links_ROMS/z_testing/Mercator/global-reanalysis-phy-001-030-daily_1993_addVars.nc"

input_bathymetry_nc_file = "/home/blaughli/tracking_project_v2/misc/z_modifiers/y_modify_forcing_nc/z_input/cmems_mod_glo_phy_my_0.083deg_static_1750287894628.nc"
...
output_model_depths_file = "/home/blaughli/tracking_project_v2/misc/z_modifiers/y_modify_forcing_nc/z_output/model_seafloor_depths_Mercator.npz"
...
with netCDF4.Dataset(input_bathymetry_nc_file,'r',format="NETCDF3_CLASSIC") as ncfile:
    ...
    sea_binary_mask = np.array(ncfile["mask"])[0,:,:]
    #sea_binary_mask = np.array(ncfile["mask"])
    bathymetry = np.array(ncfile["deptho"])
    #bathymetry = np.array(ncfile["depth"])
    lon_bathy_1D = np.array(ncfile["longitude"]) - 360
    lat_bathy_1D = np.array(ncfile["latitude"])
#ncfile.close()
...
interp = RegularGridInterpolator((lat_bathy_1D,lon_bathy_1D), sea_binary_mask)
model_grid_mask = interp(np.array((lat_model.flatten(),lon_model.flatten())).T)
model_grid_mask = np.reshape(model_grid_mask,(np.shape(lon_model)[0],np.shape(lon_model)[1]))

d = {}
d["model_grid_depths"] = model_grid_depths
d["mask_rho"] = model_grid_mask
