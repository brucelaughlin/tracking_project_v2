...
class LarvalDrifter(Lagrangian3DArray):
    """
    Extending Lagrangian3DArray with specific properties for larval drifters experiencing tidal kicks, etc
    """

    variables = Lagrangian3DArray.add_variables([
        ('kick_speed', {'dtype': np.float32,
                      'units': 'm/s',
                      'default': 0}),
        ('kick_angle', {'dtype': np.float32,
                      'units': 'radians',
                      'default': 0})])
...
class LarvalDispersal(OceanDrift):
    ...
    def horizontal_velocity_kick(self):
        ...
        self.elements.kick_speed = kick_speeds
        self.elements.kick_angle = kick_angles
