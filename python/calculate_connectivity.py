...
#def calculate_connectivity(tracking_output_dir, polygon_file_path, base_year):
...
# store local settler trajectories
...
#    timesteps_full_run = (last_settlement_day+1) * timesteps_per_day + 1 # why isn't this just taken from the dimensions of the files?
    ...
    for tracking_output_file_pre in tracking_output_files:
        ...
        # Tracking indice of stationary settlers, for removal from bins for current file
        stationary_settlers_per_box = np.zeros(num_polygons)
        stationary_settlers_per_box_per_settleTime = np.zeros((num_polygons,timesteps_settlement_window))
        ...
        # Why are there particles with no release location (I have no idea how that happens, by the way..???!)
                    ...
                    if current_settlement_boxes[ii] != 0:
                        if current_settlement_boxes[ii] == release_boxes[ii]:
                            ...
                            if particles_lon_all[ii,0] == particles_lon_all[ii,time_dex]:
                                ...
                                stationary_settlers_per_box_per_settleTime[polygon_dex, time_dex-first_settle_dex] += 1
    ...
    for ii in range(num_polygons):
        ...
        for jj in range(timesteps_settlement_window):
            pdf_arrays_settleTime[array_dex,ii,jj] -= int(stationary_settlers_per_box_per_settleTime[ii,jj])

    print('Number of stationary settlers: {}'.format(sum(stationary_settlers_per_box)))
