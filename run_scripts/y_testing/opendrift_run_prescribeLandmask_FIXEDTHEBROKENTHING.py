...
# -----------------------------------------------------------------------------
# MUST NOT USE THESE IN PRODUCTION!  JUST FOR TESTING!!!  MAKE SURE TO CHANGE CODE DOWN BELOW
# -----------------------------------------------------------------------------

#hf = "/home/blaughli/sample_files/test_forcing/aa_Mercator/onshore_uo/onshore_10days.nc"
hf = "/data04/cedwards/forcing/mercator/reanalysis12/global-reanalysis-phy-001-030-daily_1993.nc"
...
SEED_FAKE_CONE_SWITCH = False
#SEED_FAKE_CONE_SWITCH = True
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
...
#box_file_lon_lat_pre = 'list_of_arrays_of_points_in_polygons_lonlat_combined.p'  # Make this variable input.  Still using pickle
...
if test_switch_horizontal:
    ...
    #test_cell = 26
    run_day = 0
    ###for jj in range(np.shape(list_of_arrays_of_points_in_polygons_lonlat[test_cell])[1]):
    #for jj in range(1):
     
        #bottom_depth = h[points_in_boxes_ii_jj[test_cell][0,jj],points_in_boxes_ii_jj[test_cell][1,jj]]
        #depth_min = np.floor(min(min_float_depth,bottom_depth))
...
#            times.append(datetime.datetime.strptime(str(start_seed_time+datetime.timedelta(days=run_day)), '%Y-%m-%d %H:%M:%S'))
    ...
    for run_day in range(0,seed_window_length,days_between_seeds):
        #for ii in range(np.shape(list_of_arrays_of_points_in_polygons_lonlat[ii])[1]):
        for ii in range(len(list_of_arrays_of_points_in_polygons_lonlat)):
            #for jj in range(np.shape(list_of_arrays_of_points_in_polygons_lonlat[ii])[0]):
            for jj in range(np.shape(list_of_arrays_of_points_in_polygons_lonlat[ii])[1]):
                # -----------------------------------------------------------------------------------
                ...
                # -----------------------------------------------------------------------------------
                #if roms_forcing_switch:
                #    bottom_depth = h[points_in_boxes_ii_jj[ii][0,jj],points_in_boxes_ii_jj[ii][1,jj]]
                #    depth_min = np.floor(min(min_float_depth,bottom_depth))
                #else:
...
if SEED_FAKE_CONE_SWITCH:
    o.seed_elements(lon=lon_test, lat=lat_test, number=10, radius=1000,
                            time=datetime.datetime.strptime(str(start_seed_time+datetime.timedelta(days=run_day)), '%Y-%m-%d %H:%M:%S'))
else:
